...
use hospital_system;
-- 1. Create Tables
-- Tables are created in a specific order to handle Foreign Keys correctly.
...
-- PART 2: SQL QUERIES TO ANALYZE DATA

-- Q1: Find all patients born after the year 2000
SELECT * FROM patients 
WHERE dob > '2000-12-31';

-- Q2: Find all prescriptions for patient_id 5
SELECT p.prescription_id, m.name AS medication_name, p.prescribed_date
FROM prescriptions p
JOIN medications m ON p.medication_id = m.medication_id
WHERE p.patient_id = 5;

-- Q3: Show the number of appointments per month
SELECT MONTHNAME(appointment_date) AS month, COUNT(*) AS appointment_count
FROM appointments
GROUP BY MONTHNAME(appointment_date), MONTH(appointment_date)
ORDER BY MONTH(appointment_date);

-- Q4: List all medications that include the keywords 'pain' or 'infection' in description
SELECT * FROM medications
WHERE description LIKE '%pain%' OR description LIKE '%infection%';

-- Q5: Show all doctors with their hospital name
SELECT d.name AS doctor_name, h.name AS hospital_name
FROM doctors d
JOIN hospitals h ON d.hospital_id = h.hospital_id;

-- Q6: Find names, phones and appointment_dates of patients with appointments in August 2025
SELECT p.name, p.phone, a.appointment_date
FROM patients p
JOIN appointments a ON p.patient_id = a.patient_id
WHERE a.appointment_date BETWEEN '2025-08-01' AND '2025-08-31';

-- Q7: Show all room numbers and their capacities from the Neurology departments
SELECT r.room_no, r.capacity, d.name AS department_name
FROM rooms r
JOIN departments d ON r.department_id = d.department_id
WHERE d.name = 'Neurology';

-- Q8: Count how many doctors work in each hospital
SELECT h.name AS hospital_name, COUNT(d.doctor_id) AS doctor_count
FROM hospitals h
JOIN doctors d ON h.hospital_id = d.hospital_id
GROUP BY h.name;

-- Q9: Find patients who have more than 3 appointments
...
-- Q10: List appointments with patient and doctor names
SELECT a.appointment_id, p.name AS patient_name, d.name AS doctor_name, a.appointment_date
FROM appointments a
JOIN patients p ON a.patient_id = p.patient_id
JOIN doctors d ON a.doctor_id = d.doctor_id;

-- Q11: For all emergency appointments, show patient name, dob, and age-group
SELECT 
    p.name, 
    p.dob,
    CASE 
...
-- Q12: Show all departments of Green Valley Medical Center
SELECT d.name AS department_name
FROM departments d
JOIN hospitals h ON d.hospital_id = h.hospital_id
WHERE h.name = 'Green Valley Medical Center';

-- Q13: Find patients who have never had a prescription
SELECT p.name
FROM patients p
LEFT JOIN prescriptions pre ON p.patient_id = pre.patient_id
WHERE pre.prescription_id IS NULL;

-- Q14: List name, address, phone for patients who have appointments in more than one hospital
...
-- Q15: Show the latest appointment for each patient
SELECT p.name, MAX(a.appointment_date) AS latest_appointment
FROM patients p
JOIN appointments a ON p.patient_id = a.patient_id
GROUP BY p.patient_id, p.name;

-- Q16: Show the 3rd most frequently prescribed medication
SELECT m.name, COUNT(p.prescription_id) AS frequency
FROM medications m
...
-- Q17: Show all hospitals with the lowest doctor count
WITH HospitalDoctorCounts AS (
    SELECT h.name, COUNT(d.doctor_id) AS doc_count
    FROM hospitals h
    JOIN doctors d ON h.hospital_id = d.hospital_id
    GROUP BY h.name
)
SELECT name, doc_count
FROM HospitalDoctorCounts
WHERE doc_count = (SELECT MIN(doc_count) FROM HospitalDoctorCounts);

-- Q18: Find the department with the second largest room capacity in each hospital
-- Note: This ranks departments within each hospital based on total room capacity
WITH DeptCapacity AS (
    SELECT 
        h.name AS hospital_name, 
        d.name AS dept_name, 
        SUM(r.capacity) AS total_capacity,
        DENSE_RANK() OVER (PARTITION BY h.name ORDER BY SUM(r.capacity) DESC) as rn
    FROM hospitals h
    JOIN departments d ON h.hospital_id = d.hospital_id
    JOIN rooms r ON d.department_id = r.department_id
    GROUP BY h.name, d.name
)
SELECT hospital_name, dept_name, total_capacity
FROM DeptCapacity
WHERE rn = 2;